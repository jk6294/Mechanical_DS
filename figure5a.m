...
% Unit
visualize_network(XC(:,1:3,300)*sc+sh1,XC(:,4:5,300)*sc+sh1,conn,'nalpha',.5,'lalpha',.5);
visualize_network(XC(:,1:3,1)*sc+sh1,XC(:,4:5,1)*sc+sh1,conn);

% Networks
scd = .16;
DLin = linspace(min(Da(:)),max(Da(:)),nT);
for i = 1:(nR+1)
    line_coordinates(XCa(:,i:i+1,end)*sc+sh4,'lsh',(-1)^i*.005,'color',interp1(DLin,CP,Da(i,end)));
end
visualize_network(XCa(:,1:(nR+2),1)*sc+sh3,XCa(:,(nR+3):end,1)*sc+sh3,conna);
visualize_network(XCa(:,1:(nR+2),end)*sc+sh4,XCa(:,(nR+3):end,end)*sc+sh4,conna);
line([0 0 axL(1)]+axsh(1), [axL(2) 0 0]+axsh(2),'color','k','linewidth',1);
% Ticks
line([1;1].*movmean(XCa(1,1:12,end),2,'endpoints','discard')*sc+sh4(1),...
     [0;1].*ones(2,nR+1)*.02+axsh(2),'color','k','linewidth',1);
line([0;1].*ones(2,2)*.01+axsh(1),...
     [1;1].*([s 3]-Da(end,end))*scd+axsh(2)+.04,'color','k','linewidth',1);
% Points
scatter(movmean(XCa(1,1:12,end),2,'endpoints','discard')*sc+sh4(1),...
        (Da(:,end)'-Da(end,end))*scd+axsh(2)+.04, 20, interp1(DLin,CP,Da(:,end)),'s','filled');
% Text
...
text(axsh(1)-.005,(3-Da(end,end))*scd+axsh(2)+.03,'$3$',NVTextRA{:},'color',interp1(DLin,CP,3));
text(axsh(1)-.005,(0)*scd+axsh(2)+.04,'$\sqrt{3}$',NVTextRA{:},'color',interp1(DLin,CP,sqrt(3)));
...
% Lines
% Initial
line_coordinates(Xsa2(:,1:2)*sc+sh,'lsh',0.005,'color',interp1(DLin,CP,sqrt(3)));
line_coordinates(Xsa3(:,1:2)*sc+sh1,'lsh',0.005,'color',interp1(DLin,CP,sqrt(3)));
line_coordinates(Xsa2(:,end-1:end)*sc+sh,'lsh',-0.005,'color',interp1(DLin,CP,sqrt(3)));
% Open top
line_coordinates(XP1(:,1:2)*sc+sh2,'lsh',0.005,'color',interp1(DLin,CP,sqrt(3)));
line_coordinates(XP1(:,[8 13])*sc+sh2,'lsh',0.005,'color',interp1(DLin,CP,3));
line_coordinates(XP1(:,[31 32])*sc+sh2,'lsh',-0.005,'color',interp1(DLin,CP,sqrt(3)));
% Open left
line_coordinates(XP2(:,1:2)*sc+sh3,'lsh',0.005,'color',interp1(DLin,CP,3));
line_coordinates(XP2(:,[8 13])*sc+sh3,'lsh',0.005,'color',interp1(DLin,CP,sqrt(3)));
line_coordinates(XP2(:,[31 32])*sc+sh3,'lsh',-0.005,'color',interp1(DLin,CP,sqrt(3)));
% Final
line_coordinates(XP3(:,1:2)*sc+sh4,'lsh',0.005,'color',interp1(DLin,CP,3));
line_coordinates(XP3(:,[8 13])*sc+sh4,'lsh',0.005,'color',interp1(DLin,CP,3));
line_coordinates(XP3(:,[31 32])*sc+sh4,'lsh',-0.005,'color',interp1(DLin,CP,3));
...
text(xS+0*xSh,yS-1*ySh,'$\sqrt{3}$',NVTextRA{:},'color',interp1(DLin,CP,sqrt(3)));
text(xS+1*xSh,yS-1*ySh,'$\sqrt{3}$',NVTextRA{:},'color',interp1(DLin,CP,sqrt(3)));
text(xS+2*xSh,yS-1*ySh,'$\sqrt{3}$',NVTextRA{:},'color',interp1(DLin,CP,sqrt(3)));
text(xS+0*xSh,yS-2*ySh,'$3$',NVTextRA{:},'color',interp1(DLin,CP,3));
text(xS+1*xSh,yS-2*ySh,'$\sqrt{3}$',NVTextRA{:},'color',interp1(DLin,CP,sqrt(3)));
text(xS+2*xSh,yS-2*ySh,'$\sqrt{3}$',NVTextRA{:},'color',interp1(DLin,CP,sqrt(3)));
text(xS+0*xSh,yS-3*ySh,'$\sqrt{3}$',NVTextRA{:},'color',interp1(DLin,CP,sqrt(3)));
text(xS+1*xSh,yS-3*ySh,'$3$',NVTextRA{:},'color',interp1(DLin,CP,3));
text(xS+2*xSh,yS-3*ySh,'$\sqrt{3}$',NVTextRA{:},'color',interp1(DLin,CP,sqrt(3)));
text(xS+0*xSh,yS-4*ySh,'$3$',NVTextRA{:},'color',interp1(DLin,CP,3));
text(xS+1*xSh,yS-4*ySh,'$3$',NVTextRA{:},'color',interp1(DLin,CP,3));
text(xS+2*xSh,yS-4*ySh,'$3$',NVTextRA{:},'color',interp1(DLin,CP,3));
...
% Table lines
line(([1;1].*(-1:2)*xSh + xS)*sRat(pInd) + .013,...
     [.5;-4.5].*[1 1 1 1]*ySh + yS,'color','k');
line(([-1;2].*[1 1 1 1 1 1]*xSh + xS)*sRat(pInd) + .013,...
     [1;1].*(.5:-1:-4.5)*ySh + yS,'color','k');
