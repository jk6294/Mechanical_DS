...
subp = [[ 0.00  4.00  4.50 9.50];...
        [ 4.50  9.00  5.00 4.50];...
        [ 4.50  4.00  5.00 5.00];...
        [ 9.50  8.75  4.75 4.75];...
        [ 9.50  4.00  4.75 4.75];...
        [14.25 10.00  4.75 3.50];...
        [14.25  6.50  4.75 3.50];...
        [14.25  4.00  4.75 2.50];...
        [14.25  4.00  4.75 6.00];...
        [ 0.00 0.00  19.00 4.00]];
% Fontsize
FS = 10;
...
% Title
texte = '\textbf{e}\hspace{.2cm}trace desired curve';
...
while(cI < nv - nCh)
    ...
    dA = abs(dP-ds2/2);
    cI = cI+ find(diff(sign(diff(dA)))==2,1)+11;

    if(isempty(cI)); break; end
    % Find better minimum with optimization
    [thop, fV] = fminsearch(@(tho) abs(sqrt(sum((f(tho)*scC-xP(:,i)).^2))-ds2/2),thv(cI),options);
...
cVecLS = linspace(min(cVec)-.0,max(cVec)+.0,size(CS,1));
cVecLP = linspace(min(cVec)-.0,max(cVec)+.0,size(CP,1));
for i = 1:size(xP,2)-2
    fill(xP(1,[0:2 0]+i)*sc+shl(1),xP(2,[0:2 0]+i)*sc+shl(2),...
         interp1(cVecLS,CS,cVec(i)),'linewidth',.1);
    line_coordinates(xP(:,[0 2]+i)*sc+shl,'lw',1.5,'lSh',0,...
        'color',interp1(cVecLP,CP,cVec(i)));
end
% Text
texte2 = 'isocelese triangle tesselation';
text(labX,subp(pInd,4)+labY,texte2,NVTitle{:});
% Axis
ax = [0 sRat(pInd) 0 1];
axis(ax);
set(gca,'visible',0);
...
subplot('position',subpN(pInd,:)); cla;
% Text
texte3 = 'combine units in tesselation';
text(labX,subp(pInd,4)+labY,texte3,NVTitle{:});
% Solve for solutoin spaces
...
[Xscc,Xucc,conncc,CSSc] = network_chain_x(Xs1(:,1:2),Xuc,IC',interp1(cVecLS,CS,cVec));
sc = .17; 
XsP = Xscc(:,:,1)*sc+sh1;
XuP = Xucc(:,:,1)*sc+sh1;
shI = [.5;0];
...
nSh = 6.6;
for i = 5:7
    scatter(linspace(0,shI(1),nP)+XsP(1,i+1)+shI(1)*(i-nSh),ones(1,nP)*XsP(2,i+1),.2,[1 1 1]*.8);
    scatter(linspace(0,shI(1),nP)+XsP(1,i+2)+shI(1)*(i-nSh),ones(1,nP)*XsP(2,i+2),.2,[1 1 1]*.8);
    visualize_network(XsP(:,[0:2]+i)+shI*(i-nSh),XuP(:,(-1:0)+2*i)+shI*(i-nSh),conn,...
                      'ucolor',CS(1+14*(mod(i,2)),:));
end
...
% Title
textf = '\textbf{f}\hspace{.2cm}conformational motion';
...
load clover;
% Correct curvature
XCcc = XCcc(:,:,1:3467) - XCcc(:,size(Xscc,2),1:3467);
for i = 1:size(XCcc,3)
    Rz = rotz(atan2d(diff(XCcc(2,[-2 0]+size(Xscc,2),i)),diff(XCcc(1,[-2 0]+size(Xscc,2),i))));
    Rz2 = rotz(0);
    XCcc(:,:,i) = Rz2(1:2,1:2)*Rz(1:2,1:2)'*XCcc(:,:,i);
end
% XCcc(2,:,:) = -XCcc(2,:,:);
pI = [20 200 1325  2740 size(XCcc,3) 1];
pIL = floor([.1 .2 .5 .8 1 1]*size(conncc,1));
for i = 1:length(pI)
    na = 1; la = 1;
    if(i<length(pI)); na = .2; la = .2; end
    if(i==length(pI)-1)
        Rz = rotz(45); Rz = Rz(1:2,1:2);
    else
        Rz = eye(2);
    end
    visualize_network(Rz*XCcc(:,unique(conncc(1:pIL(i),1)),pI(i))*sc+shl,...
                      Rz*XCcc(:,unique(conncc(1:pIL(i),2)),pI(i))*sc+shl,...
                      conncc(1:pIL(i),:) + [0 max(conncc(1:pIL(i)))-size(Xscc,2)],...
                      'lalpha',la,'nalpha',na,'msize',3,'ucolor',CSSc(1:length(unique(conncc(1:pIL(i),2))),:));
end

% Final configuration
shl = [4.95;0.15];
Rz = rotz(45); Rz = Rz(1:2,1:2);
visualize_network(Rz*XCcc(:,1:size(Xscc,2),end)*sc+shl,...
                  Rz*XCcc(:,(1:size(Xucc,2))+size(Xscc,2),end)*sc+shl,conncc,'msize',2,'ucolor',CSSc);
for i = 1:size(xP,2)-2
    line_coordinates(Rz*XCcc(:,[0 2]+i,end)*sc+shl,'lw',1.5,'lSh',0,...
        'color',interp1(cVecLP,CP,cVec(i)));
end
