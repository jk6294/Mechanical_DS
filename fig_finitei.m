...
text(.01,yPos-.035,'length $l_k,l_{k+1}$',NVTextR{:});
...
text(Xs1(1,3)*sc+sh1(1,1)+.015,Xs1(2,3)*sc+sh1(2,1),'$3$',...
     'fontsize',FS*4/4,'horizontalalignment','left','color',o*gr^4);
% Edge lengths
text(.09,sh1(2,1)+.025,'$l_k$',NVTextR{:});
text(.3,sh1(2,1)+.025,'$l_{k+1}$',NVTextR{:});
...
text(.89,0.045,'$l_k$',NVTextR{:});
text(.03,.91,'$l_{k+1}$',NVTextR{:});
