...
% Text
textc = '\textbf{c}\hspace{7mm}combine two networks by joining nodes';
...
arrow([0 .33]+.35+sh3(1),[0 0]+sh3(2)-.03,sRat(pInd),'linewidth',1.5,...
      'headwidth',10,'headlength',10,'lp',.9,'color',o*gr^2);
arrow([0 .13]+.03+sh1(1),[0 -.13*s]-.15+sh1(2),sRat(pInd),'linewidth',1.5,...
      'headwidth',10,'headlength',10,'lp',.9,'color',o*gr^2);
arrow([0 .13]+.03+sh2(1),[0 -.13*s]-.15+sh2(2),sRat(pInd),'linewidth',1.5,...
      'headwidth',10,'headlength',10,'lp',.9,'color',o*gr^2);
...
% Text arrows
text(.233,.43,'open $d_{\mathrm{in},1}$',NVTextR{:},'color',[1 1 1]*.8,...
     'rotation',-60);
text(.61,.43,'open $d_{\mathrm{in},1}$',NVTextR{:},'color',[1 1 1]*.8,...
     'rotation',-60);
text(.344,.61,'open $d_{\mathrm{in},2}$',NVTextR{:},'color',[1 1 1]*.8);
...
for i = 1:3
    for j = 1:3
        text((xS+(i-1)*xSh+(j-1)*xySh+txSh)/sP,...
             yS-(j-1)*s*xySh+tySh,tM{i,j},NVTextRA{:},...
             'color', tC{i,j});
    end
end
text((xS+1.3*xSh+txSh)/sP,yS+s*xySh+tySh,'$d_{\mathrm{in},2}$',NVTextRA{:});
text((xS+1.5*xySh-xSh+txSh)/sP,yS-1.5*s*xySh+tySh,'$d_{\mathrm{in},1}$',...
     NVTextRA{:});
text(.853,1,'truth table: $d_{\mathrm{out}}$',NVTextH{:});
% Table lines
line([0;3]*xySh+2*xSh + xS,[0;-3]*s*xySh + yS,'color','k','clipping',0);
line([0;3]*xSh+2*xySh + xS,[-s;-s].*2*xySh + yS,'color','k','clipping',0);
line([1;3]*xySh+0*xSh + xS,[-1;-3]*s*xySh + yS,'color','k','clipping',0);
line([1;3]*xSh+0*xySh + xS,[-s;-s].*0*xySh + yS,'color','k','clipping',0);
line([-1;3]*xySh+[1 3]*xSh + xS,[1;-3]*s*xySh + yS,'color','k',...
     'linewidth',1,'clipping',0);
line([-1;3]*xSh+[1 3]*xySh + xS,[-s;-s].*[1 3]*xySh + yS,'color','k',...
     'linewidth',1,'clipping',0);
line([-1;3]*xySh-xSh + xS,[1;-3]*s*xySh + yS,'color',o*dgr,'linewidth',1);
line([-1;3]*xSh-xySh + xS,[-s;-s].*-xySh + yS,'color',o*dgr,'linewidth',1);

axis([0 sRat(pInd) 0 1]);
set(gca,'visible',0);

% Text
textd = '\textbf{b}\hspace{2mm} mechanical AND gate';
