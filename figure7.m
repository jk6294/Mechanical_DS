...
xlz = linspace(.24, .39, 1000);
ylz = linspace(-1.025,-0.975,500);
slMz = zeros(length(xlz),length(ylz));
for i = 1:length(xlz)
    for j = 1:length(ylz)
        slMz(i,j) = slfun(xlz(i),ylz(j));
    end
end
...
subp = [[ 0.00  4.75  4.75  4.75];...
        ...
        [ 5.00  6.40  7.00  3.10];...
        [ 5.00  3.20  7.00  3.10];...
        [12.00  6.40  7.00  3.10];...
        [12.00  3.20  7.00  3.10];...
        [ 5.00  0.00 14.00  3.10]];
...
% Sample phase space
nRep = 8;
xpI = [.25 .3715 .38264 .38398156];
Xub = zeros(4,2,length(xpI));
Xuc = zeros(2,4,length(xpI));
Xsa = zeros(2,2*nRep+2,length(xpI));
Xua = zeros(2,4*nRep,length(xpI));

for i = 1:length(xpI)
    Xu = [[0; -1] [1;-l/h]*xpI(i)];
    [Xub(:,:,i),~] = construct_network(Xs,Xf,Xu,conn,0,1);
    Xuc(:,:,i) = [Xub(3:4,:,i) [Xub(1,:,i)+L-l;-Xub(2,:,i)+h]];
    [Xsa(:,:,i),Xua(:,:,i),conna] = tesselate_network_old(Xsc,Xuc(:,:,i), connc, [2*L;0], [nRep;1]);
end

% Simulate
XCa1 = [Xsa(:,:,1) Xua(:,:,1)];
% [XCa1,fC1] = sim_motion8(Xsa(:,:,1),Xua(:,:,1),conna,.01,143,[Xsa(:,:,1),Xua(:,:,1)],0);
[XCa2,fC2] = sim_motion8(Xsa(:,:,2),Xua(:,:,2),conna,.01,61,-[Xsa(:,:,2),Xua(:,:,2)],0);
[XCa3,fC3] = sim_motion8(Xsa(:,:,3),Xua(:,:,3),conna,.01,143,-[Xsa(:,:,3),Xua(:,:,3)],0);
% [XCa4,fC4] = sim_motion8(Xsa(:,:,4),Xua(:,:,4),conna,.01,143,-[Xsa(:,:,4),Xua(:,:,4)],0);
Da1 = squeeze(sum(diff(XCa1(:,1:size(Xsa,2),:),1,2).^2));
Da2 = squeeze(sum(diff(XCa2(:,1:size(Xsa,2),:),1,2).^2));
Da3 = squeeze(sum(diff(XCa3(:,1:size(Xsa,2),:),1,2).^2));
% Da4 = squeeze(sum(diff(XCa4(:,1:size(Xsa,2),:),1,2).^2));
...
hold on; 
C1 = contour(yl,xl,abs(slM),[1 1],'color',interp1(SLin,CP,tanh(1)),'linewidth',2);
plot(yl,1/4*ones(1,length(yl)),':','linewidth',1,'color',CP(1,:)); 
plot(yl,yl./(1-yl),':','linewidth',1,'color',CP(1,:)); 
plot(yl,-yl./(1+yl),':','linewidth',1,'color',CP(1,:)); 
...
line([min(yl) min(yl) max(yl) max(yl) min(yl) min(yl)]+[-1 -1 1 1 -1 -1]*nSc,...
     ...
     'color', 'k', 'linewidth',2);
line([min(ylz) min(ylz) max(ylz) max(ylz) min(ylz) min(ylz)]+[-1 -1 1 1 -1 -1]*nSc,...
     [min(xlz) max(xlz) max(xlz) min(xlz) min(xlz) max(xlz)]+[-1 1 1 -1 -1  1]*nSc,...
     'color', 'r', 'linewidth',2);
hold off;

% Text
text(labX,subp(pInd,4)+labY,'\textbf{a} \hspace{1mm}full phase diagram',NVTitle{:});
text(.02,.31,'unstable',NVTextR{:});
text(.02,.69,'stable',NVTextR{:});
text(.5,-.065,'$y_1$',NVTextH{:});
text(0,-.07,'$-3/2$',NVTextH{:});
text(1,-.07,'$0$',NVTextH{:});
text(-.07,.5,'$x_2$',NVTextH{:});
text(-.07,0.05,'$\frac{-3}{4}$',NVTextH{:});
text(-.07,.95,'$\frac{3}{4}$',NVTextH{:});
...
% Contour
colormap(CP);
imagesc(ylz,xlz,tanh(abs(slMz)),'alphadata',.6);
hold on; 
C1 = contour(ylz,xlz,abs(slMz),[1 1],'color',interp1(SLin,CP,tanh(1)),'linewidth',2);
plot(ylz,1/4*ones(1,length(ylz)),':','linewidth',1,'color',CP(1,:));
...
for i = 1:length(xpI)
    plot(-1,xpI(i),mL{i});
    visualize_network(Xsc*sc+sh+[-1+nShx*(-1)^i;nShy*(i-1)],...
                      Xuc(:,:,i)*sc+sh+[-1+nShx*(-1)^i;nShy*(i-1)],connc);
end

hold off;

% Colorbar
hb = colorbar('location', 'east','position',[0.2 .055 .021 .39]);
delete(findall(gcf,'type','annotation'));
...
annotation('line',[.7165 .7355]-.5155, [1 1]*.351, 'Units','Normalize',...
           'color',interp1(SLin,CP,tanh(1)),'linewidth',2);
annotation('line',[.7165 .7355]-.5155, [1 1]*.06, 'Units','Normalize',...
           'color',interp1(SLin,CP,tanh(0)),'linewidth',2);

% Text
text(labX,subp(pInd,4)+labY,'\textbf{b} \hspace{1mm}partial phase diagram',NVTitle{:});
text(.27,-.065,'$y_1$',NVTextH{:});
text(-.08,.5,'$x_2$',NVTextH{:});
text(1.07,0.0,'$0$',NVTextH{:});
text(1.07,.77,'$1$',NVTextH{:});
text(1.07,1,'$\infty$',NVTextH{:});
annotation(gcf,'textarrow',[1 1]*.25, [1 1]*.2,'String','$|\delta d_3 / \delta d_1|$',...
    'HeadStyle', 'none', 'LineStyle', 'none',NVTextH{:}, 'TextRotation',90,...
    'interpreter','latex','color','w');

% Axes
axis([min(ylz) max(ylz)+pDx-pDy min(xlz) max(xlz)] + [-1 1 -1 1]*nSc);
set(gca,'xtick',[],'ytick',[],'ydir','normal','visible',0);
...
sc = .18;
sh = [.2;.4];
visualize_network(XCa1(:,1:size(Xsa,2),end)*sc+sh,...
                  XCa1(:,(1:size(Xua,2))+size(Xsa,2),end)*sc+sh,conna);
text(labX,subp(pInd,4)+labY,'\textbf{c} \hspace{1mm}networks',NVTitle{:});
axis([0 sRat(pInd) 0 1]);
set(gca,'visible',1);

pInd = 4;
subplot('position',subpN(pInd,:)); cla;
visualize_network(XCa2(:,1:size(Xsa,2),end)*sc+sh,...
                  XCa2(:,(1:size(Xua,2))+size(Xsa,2),end)*sc+sh,conna);
text(labX,subp(pInd,4)+labY,'\textbf{c} \hspace{1mm}networks',NVTitle{:});
axis([0 sRat(pInd) 0 1]);
set(gca,'visible',1);
...
pInd = 5;
subplot('position',subpN(pInd,:)); cla;
visualize_network(XCa3(:,1:size(Xsa,2),end)*sc+sh,...
                  XCa3(:,(1:size(Xua,2))+size(Xsa,2),end)*sc+sh,conna);
text(labX,subp(pInd,4)+labY,'\textbf{c} \hspace{1mm}networks',NVTitle{:});
axis([0 sRat(pInd) 0 1]);
set(gca,'visible',1);
...
% hold on;
% % Initial network
% line_coordinates(Xs(:,1:2)*sc+sh1,'lSh',lSh,'color',interp1(DLin,CP,ds(1)));
% line_coordinates(Xs(:,2:3)*sc+sh1,'lSh',lSh,'color',interp1(DLin,CP,ds(2)));
% visualize_network(Xs*sc+sh1,[],[1 1],'scolor',[1 1 1]*.8,'msize',mSs);
% % Final network
% line_coordinates(Xf(:,1:2)*sc+sh2,'lSh',lSh,'color',interp1(DLin,CP,df(1)));
% line_coordinates(Xf(:,2:3)*sc+sh2,'lSh',lSh,'color',interp1(DLin,CP,df(2)));
% visualize_network(Xf*sc+sh2,[],[1 1],'scolor',[1 1 1]*0,'bcolor',[1 1 1]*.0,'msize',mSf);
% % Solution space
% visualize_conic_finite(Xs*sc+sh,Xf*sc+sh,R*sc+sh,'overlay',.99,'ucolori',[1 1 1]*.8);
% visualize_network(Xs*sc+sh,[],[1 1],'scolor',[1 1 1]*.8,'msize',mSs);
% visualize_network(Xf*sc+sh,[],[1 1],'scolor',[1 1 1]*0,'bcolor',[1 1 1]*.0,'msize',mSf);
% visualize_network([Xu1 Xu2]*sc+sh,[],[1 1],'scolor',interp1(SLin,CP,0),'msize',mSf);
% % Legend
% visualize_network([0;0]+shl,[],[1 1],'scolor',[1 1 1]*.8,'msize',mSs);
% visualize_network([0.31;0]+shl,[],[1 1],'scolor',[1 1 1]*0,'msize',mSf);
% line([0 .1/sc]*sc+shl(1)+.54, [0 0]*sc+shl(2),'linewidth',1,'color',[1 1 1]*.8);
% hold off;
% 
% % Text
...
% text(0,sh1(2)+.1,'$d_1$',NVTextR{:},'color',interp1(DLin,CP,ds(1)));
% text(.3,sh1(2)+.1,'$d_2$',NVTextR{:},'color',interp1(DLin,CP,ds(2)));
% text(.51,sh2(2)+.1,'$d_1$',NVTextR{:},'color',interp1(DLin,CP,df(1)));
% text(.75,sh2(2)+.1,'$d_2$',NVTextR{:},'color',interp1(DLin,CP,df(2)));
% text(sh(1),sh(2)-.23,'$y_1$',NVTextH{:});
% text(sh(1)+.2,sh(2)-.23,'$x_2$',NVTextH{:});
% % text(-.08,sh(2)-.08,'add',NVTextR{:},'color',[1 1 1]*.6);
% text(-.08,sh(2)-.16,'add nodes',NVTextR{:},'color',[1 1 1]*.6);
% text(-.08,sh(2)-.24,'at $y_1,x_2$',NVTextR{:},'color',[1 1 1]*.6);
% 
% 
% % Axes
% axis([0 sRat(pInd) 0 1]);
...
% line([1;1].*df, [0;.04].*[1 1]+axL(3),'linewidth',.5,'color','k');
% line([0;.04].*[1 1]+axL(1), [1;1].*df,'linewidth',.5,'color','k');
% text(.86,.035,'$d_1$',NVTextR{:});
% text(-.07,.5,'$d_2$',NVTextR{:});
% text(.68,-.04,'$\sqrt{10}/2$',NVTextRA{:},'color',interp1(DLin,CP,ds(1)));
% text(.2,-.04,'$\sqrt{2}/2$',NVTextRA{:},'color',interp1(DLin,CP,ds(2)));
% 
% % Plot
% lSh = .02;
% % Map
% plot(axL(1:2),axL(3:4),'color',[1 1 1]*.8,'linewidth',.5);
% plot(D(1,:),D(2,:),'color','k','linewidth',1);
% % Points
% [~,fpN] = min(abs(D(1,:)-D(2,:)));
% dfp = D(1,fpN);
% plot(ds,df,'ks','linewidth',3,'markersize',3);
% line_coordinates(XCa(:,1:2,1)*sc+ds'+sh1,'lSh',lSh,'color',interp1(DLin,CP,ds(1)));
% line_coordinates(XCa(:,2:3,1)*sc+ds'+sh1,'lSh',lSh,'color',interp1(DLin,CP,ds(2)));
% visualize_network(XCa(:,1:3,1)*sc+ds'+sh1,XCa(:,4:5,1)*sc+ds'+sh1,conn,...
%                   'ucolor',interp1(SLin,CP,tanh(0)));
% line_coordinates(XCa(:,1:2,end)*sc+df'+sh2,'lSh',lSh,'color',interp1(DLin,CP,df(1)));
% line_coordinates(XCa(:,2:3,end)*sc+df'+sh2,'lSh',lSh,'color',interp1(DLin,CP,df(2)));
% visualize_network(XCa(:,1:3,end)*sc+df'+sh2,XCa(:,4:5,end)*sc+df'+sh2,conn,...
%                   'ucolor',interp1(SLin,CP,tanh(0)));
% plot(dfp,dfp,'ks','linewidth',3,'markersize',3);
% line_coordinates(XCa(:,1:2,fpN)*sc+dfp+sh3,'lSh',lSh,'color',interp1(DLin,CP,dfp));
% line_coordinates(XCa(:,2:3,fpN)*sc+dfp+sh3,'lSh',lSh,'color',interp1(DLin,CP,dfp));
...
%                 Xsa(2,1+i)*sc+sh(2)+shL(2)*i+zeros(1,10),.2,[1 1 1]*.9,'clipping',0);
%         scatter(Xsa(1,2+i)*sc+sh(1)+shL(1)*i+dotX,...
%                 Xsa(2,2+i)*sc+sh(2)+shL(2)*i+zeros(1,10),.2,[1 1 1]*.9,'clipping',0);
%     else
%         scatter(Xsa(1,1+i)*sc+sh(1)+shL(1)*i+dotX,...
%                 Xsa(2,1+i)*sc+sh(2)+shL(2)*i+zeros(1,10),.2,...
%                 [1 1 1].*linspace(.9,1,length(dotX))','clipping',0);
%         scatter(Xsa(1,2+i)*sc+sh(1)+shL(1)*i+dotX,...
%                 Xsa(2,2+i)*sc+sh(2)+shL(2)*i+zeros(1,10),.2,...
%                 [1 1 1].*linspace(.9,1,length(dotX))','clipping',0);
%     end
%     line_coordinates(Xsa(:,(0:1)+i)*sc+sh+shL*i,'lSh',lSh*(-1)^(i-1),...
%                      'color',interp1(DLin,CP,ds(mod(i,2)+1)));
%     visualize_network(Xsa(:,(0:2)+i)*sc+sh+shL*i,Xua(:,(-1:0)+2*i)*sc+sh+shL*i,conn,...
%                      'ucolor',interp1(SLin,CP,0));
% end
% 
% % Box
% bD = [-.3 .95 .1 .79];
% line(bD([1 1 2 2 1]), bD([3 4 4 3 3]), 'color', [1 1 1]*.8, 'linewidth',.5,'clipping',0);
% line(bD([1 1 2 2 1])+1.32, bD([3 4 4 3 3]), 'color', [1 1 1]*.8, 'linewidth',.5,'clipping',0);
% hold off;
% 
% % Text
% text(labX-.5,subp(pInd,4)+labY,'\textbf{c} \hspace{1mm}repeats every 2 units',NVTitle{:});
% text(-.1,.65,'$d_1$',NVTextR{:},'color',interp1(DLin,CP,df(1)));
% text(.2,.25,'$d_2$',NVTextR{:},'color',interp1(DLin,CP,ds(1)));
% text(.46,.65,'$d_3$',NVTextR{:},'color',interp1(DLin,CP,df(1)));
% text(.76,.25,'$d_4$',NVTextR{:},'color',interp1(DLin,CP,ds(1)));
% 
% % Axes
% axis([0 sRat(pInd) 0 1]);
% set(gca,'visible',0);
...
%                   'nalpha',.3,'lalpha',.3,'ucolor',interp1(SLin,CP,0));
...
% text(.5,.05,'$|\delta d_3 / \delta d_1|$',NVTextH{:});
% text(.1,.67,'$\delta d_1$',NVTextH{:});
% text(.9,.28,'$\delta d_3$',NVTextH{:});
% 
% % Axes
% axis([0 sRat(pInd) 0 1]);
% set(gca,'visible',0);
...
%     visualize_network(Xsp*sc+sh,Xup0*sc+sh,conn,'ucolor',CP(1,:),'msize',3,...
%         'nalpha',0.3,'lalpha',0.3); 
%     visualize_network(Xf*sc+sh,Xupf*sc+sh,conn,'ucolor',CP(1,:),'msize',3,...
...
%     visualize_network(Xsc*sc+sh,[Xupf Xup0]*sc+sh,connc,'ucolor',interp1(SLin,CP,tanh(1)),...
...
% annotation(gcf,'textarrow',[1 1]*.32, [1 1]*.4,'String','$|\delta d_3 / \delta d_1|$',...
%     'HeadStyle', 'none', 'LineStyle', 'none',NVTextH{:}, 'TextRotation',90,...
%     'interpreter','latex','color','w');
% 
% % Axes
...
%     Xsp = [Xs(1,:); -Xs(2,:)+h] + [L-l;0];
%     if(i < length(nP))
%         visualize_network(Xsc*sc+sh,[Xupf Xup0]*sc+sh,[1 1],'ucolor',interp1(SLin,CP,tanh(1)),...
...
%     Xsp = [Xs(1,:); -Xs(2,:)+h] + [L-l;0];
%     if(i < length(nP))
%         visualize_network(Xf*sc+sh,Xupf*sc+sh,[1 1],'ucolor',CP(1,:),'msize',4,...
%             'nalpha',.1*i,'lalpha',.1*i); 
%     else
%         visualize_network(Xsp*sc+sh,Xup0*sc+sh,conn,'ucolor',CP(1,:),'msize',4,...
%             'nalpha',.1,'lalpha',.1); 
%         visualize_network(Xf*sc+sh,Xupf*sc+sh,conn,'ucolor',CP(1,:),'msize',4,...
%             'nalpha',1,'lalpha',1); 
%         XfM = mean(Xf(:,2:3),2); XfD = diff(Xf(:,2:3),1,2);
%         line_coordinates(([-XfD XfD]*.8+XfM)*sc+sh,'lSh',lSh,...
%                          'style','-','lw',.5,'color',[.8 .6 0]);
%     end
% end
...
%                          'style','-','lw',.5,'color',[.8 .6 0],'lalpha',.3);
%     else
%         visualize_network(Xsp*sc+sh,Xup0*sc+sh,conn,'ucolor',CP(1,:),'msize',4,...
%             'nalpha',.1,'lalpha',.1); 
%         visualize_network(Xf*sc+sh,Xupf*sc+sh,conn,'ucolor',CP(1,:),'msize',4,...
...
%     if(i < length(nP))
%         visualize_network(Xsp*sc+sh,Xup0*sc+sh,[1 1],'ucolor',CP(1,:),'msize',4,...
...
% text(.2,.915,'$|\delta d_3/\delta d_1| = 1$',NVTextH{:});
% text(labX+3.1,subp(pInd,4)+labY,'\textbf{g} \hspace{3mm}superstable',NVTitle{:});
% text(.78,.915,'$|\delta d_3/\delta d_1| = 0$',NVTextH{:});
% 
% % Axes
% axis([0 sRat(pInd) 0 1]);
...
%     line_coordinates(XCaaP(:,(0:1)+i),'lsh',.018*(1)^i,...
%                      'color',interp1(DLin,CP,Da(i,plI)));
% end
% visualize_network(XCaaP(:,1:size(Xsa,2)),...
%                   XCaaP(:,(1:size(Xua,2))+size(Xsa,2)),...
%                   conna,'msize',3,'ucolor',interp1(SLin,CP,tanh(0)));
% % Distance plot
% lCo = [ 0.15  0.15  2.41;...
%         0.45 -0.04 -0.04];
% line(lCo(1,:),lCo(2,:),'linewidth',0.5,'color',[1 1 1]*.7,'clipping',0);
% scatter(XDa,Da(:,plI)'*psc+pSh,3,interp1(DLin,CP,Da(:,plI)),...
%         'marker','s','linewidth',2);
% line([1;1].*XDa, [0;.04]+ones(1,size(Xsa,2)-1)*lCo(2,2),...
%      'linewidth',.5,'color',[1 1 1]*.7,'clipping',0);
% line([0;.04]+[1 1]*lCo(1,1),[1;1].*ds*psc+pSh,...
%      'linewidth',.5,'color',[1 1 1]*.7,'clipping',0);
% 
% % Text
% text(labX,subp(pInd,4)+labY+.15,'\textbf{h} \hspace{1mm}stable limit cycle',NVTitle{:});
% text(.05,ds(1)*psc+pSh,'$\frac{\sqrt{10}}{2}$',NVTextRA{:},'color',interp1(DLin,CP,ds(1)));
% text(.05,ds(2)*psc+pSh,'$\frac{\sqrt{2}}{2}$',NVTextRA{:},'color',interp1(DLin,CP,ds(2)));
