...
sc = .17;                       % Scale networks
sh = [0.5;0.85];               % Shift networks
thR = linspace(15,42.2683,nSS); % Angles of solution spaces to draw
R2 = [-1 1; -1 1]*3.2;          % Range of solution space to draw

% Draw solution space
for i = 1:nSS
    Xfp = sq*2*1.7*[[-sind(thR(i));-cosd(thR(i))] [0;0],...
               [sind(thR(i));-cosd(thR(i))]] + [0;Xf(2,2)];
...
pInd = 1;
sc = .15;                       % Scale networks
sh = [0.5;.85];                 % Shift networks
R2 = [-1 1; -1 1]*3.5;          % Range of solution space to draw
for i = [13 8 3 1]
    pInd = pInd + 1;
    subplot('position',subpN(pInd,:)); cla; hold on;
    Xfp = sq*2*1.7*[[-sind(thR(i));-cosd(thR(i))] [0;0],...
