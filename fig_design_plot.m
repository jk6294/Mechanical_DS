...
line_coordinates(Xs(:,[1 3])*sc+sh,'lSh',-lSh1,'color',C2a);
line_coordinates(Xf(:,1:2)*sc+sh,'lSh',lSh2,'color',C1c);
line_coordinates(Xf(:,2:3)*sc+sh,'lSh',lSh2,'color',C1c);
line_coordinates(Xf(:,[1 3])*sc+sh,'lSh',-lSh2,'color',C2b);
% Draw network
visualize_network(Xf*sc+sh,[],[1 1],'msize',7,'scolor',[1 1 1]);
...
text(Xs(1,1)*sc+sh(1)+.07,Xs(2,1)*sc+sh(2)+.03,'$1$',NVTexth{:},'color',o*gr,'fontsize',8);
text(Xs(1,2)*sc+sh(1),Xs(2,2)*sc+sh(2)-.09,'$2$',NVTexth{:},'color',o*gr,'fontsize',8);
text(Xs(1,3)*sc+sh(1)-.07,Xs(2,3)*sc+sh(2)+.03,'$3$',NVTexth{:},'color',o*gr,'fontsize',8);
% text(Xs(1,1)*sc+sh(1)+.13,Xs(2,1)*sc+sh(2)+.13,'$l^\bullet$',NVTexth{:},'color',C1a);
% text(Xs(1,3)*sc+sh(1)-.12,Xs(2,3)*sc+sh(2)+.13,'$l^\bullet$',NVTexth{:},'color',C1a);
% text(Xs(1,2)*sc+sh(1),Xs(2,1)*sc+sh(2)-.09,'$c^\bullet$',NVTexth{:},'color',C2a);
% Axes
...
subplot('position',subpN(pInd,:)); cla; hold on;
% Lengths
line_coordinates(Xs(:,1:2)*sc+sh,'lSh',lSh1,'lw',1,'color',C1a);
line_coordinates(Xs(:,2:3)*sc+sh,'lSh',lSh1,'lw',1,'color',C1a);
line_coordinates(Xs(:,[1 3])*sc+sh,'lSh',-lSh1,'lw',1,'color',o*gr);
% Draw network
visualize_network(Xs*sc+sh,[],[1 1]);
% Text
text(labX,subp(pInd,4)+labY,'\textbf{b}',NVTitle{:});
text(subp(pInd,3)/2-fMarg(1),subp(pInd,4)+labY,'start position',NVTitleH{:});
text(Xs(1,1)*sc+sh(1)+.07,Xs(2,1)*sc+sh(2)+.03,'$1$',NVTexth{:},'color',o*gr,'fontsize',8);
text(Xs(1,2)*sc+sh(1),Xs(2,2)*sc+sh(2)-.08,'$2$',NVTexth{:},'color',o*gr,'fontsize',8);
text(Xs(1,3)*sc+sh(1)-.07,Xs(2,3)*sc+sh(2)+.03,'$3$',NVTexth{:},'color',o*gr,'fontsize',8);
text(Xs(1,1)*sc+sh(1)+.03,Xs(2,1)*sc+sh(2)+.2,'$l^\bullet$',NVTexth{:},'color',C1a);
text(Xs(1,3)*sc+sh(1),Xs(2,3)*sc+sh(2)+.2,'$l^\bullet$',NVTexth{:},'color',C1a);
text(Xs(1,2)*sc+sh(1),Xs(2,1)*sc+sh(2)-.1,'$c^\bullet$',NVTexth{:},'color',o*gr);
% Axes
axis([0 sRat(pInd) 0 1]);
set(gca,'visible',0,'xtick',[],'ytick',[]);
drawnow;
...
line_coordinates(Xf(:,[1 3])*sc+sh,'lSh',-lSh1,'lw',1,'color',o*gr);
% Draw network
visualize_network(Xf*sc+sh,[],[1 1]);
% Text
text(labX,subp(pInd,4)+labY,'\textbf{c}',NVTitle{:});
text(subp(pInd,3)/2-fMarg(1),subp(pInd,4)+labY,'end position',NVTitleH{:});
text(Xf(1,1)*sc+sh(1)+.07,Xf(2,1)*sc+sh(2)+.03,'$1$',NVTexth{:},'color',o*gr,'fontsize',8);
text(Xf(1,2)*sc+sh(1),Xf(2,2)*sc+sh(2)-.08,'$2$',NVTexth{:},'color',o*gr,'fontsize',8);
text(Xf(1,3)*sc+sh(1)-.07,Xf(2,3)*sc+sh(2)+.03,'$3$',NVTexth{:},'color',o*gr,'fontsize',8);
text(Xf(1,1)*sc+sh(1)+.03,Xf(2,1)*sc+sh(2)+.4,'$l^\circ$',NVTexth{:},'color',C1c);
text(Xf(1,3)*sc+sh(1),Xf(2,3)*sc+sh(2)+.4,'$l^\circ$',NVTexth{:},'color',C1c);
text(Xf(1,2)*sc+sh(1),Xf(2,1)*sc+sh(2)-.1,'$c^\circ$',NVTexth{:},'color',o*gr);
% Axes
axis([0 sRat(pInd) 0 1]);
set(gca,'visible',0,'xtick',[],'ytick',[]);

% Arrow
sCo11 = [0.00 1.00; 0 0] .* [.4;1] + [-.35;.58];
plot_spline(sCo11,'head',1,'headpos',1,'ratio',sRat(pInd),...
            'linewidth',.5,'headwidth',5,'headlength',5,'color',o*gr);
text(-.15,.75,'3 conformational',NVTexth{:},'color',o*gr);
text(-.15,.65,'motions',NVTexth{:},'color',o*gr);

drawnow;


%% d-f: solution space, node placement, network
% Parameters
sc = .23;
...
visualize_network(Xs*sc+sh,Xu*sc+sh,conn,'ucolor',CP2a);
% Text
shLx = [-2  14  20 -2  11  14]*.01;
shLy = [ 0  2  -6   0 -5  -4]*.01;
for i = 1:size(conn,1)
...
text(subp(pInd,3)/2-fMarg(1),subp(pInd,4)+labY,'on solution space',NVTitleH{:});
% Axes
axis([0 sRat(pInd) 0 1]);
set(gca,'visible',0,'xtick',[],'ytick',[]);
drawnow;
...
line_coordinates(Xf(:,1:2)*sc+sh,'lSh',lSh1,'lw',1,'color',C1c);
line_coordinates(Xf(:,2:3)*sc+sh,'lSh',lSh1,'lw',1,'color',C1c);
visualize_network(XC(:,1:3,end)*sc+sh,XC(:,4:5,end)*sc+sh,conn,'ucolor',CP2a);
% Text
...
plot_spline(sCo11,'head',1,'headpos',1,'ratio',sRat(pInd),...
            'linewidth',.5,'headwidth',5,'headlength',5,'color',o*gr);
text(-.15,.75,'1 conformational',NVTexth{:},'color',o*gr);
text(-.15,.65,'motion',NVTexth{:},'color',o*gr);
...
plInd = ceil([1 25 45 68 size(XC,3)]);
plSh = [ .22  .22  .22 .22 .22;...
        -.11 -.07 -.00 .05 .05];
lSh = -.015;
...
scatter(D(1,plInd([1 end])),D(2,plInd([1 end])),80,...
       [interp1(DLin,CP1,D(1,plInd(1)));interp1(DLin,CP1,D(1,plInd(end)))],...
       'filled','marker','s','linewidth',.5);
scatter(D(1,plInd([1 end])),D(2,plInd([1 end])),40,ones(2,3),'marker','s','linewidth',.1);
